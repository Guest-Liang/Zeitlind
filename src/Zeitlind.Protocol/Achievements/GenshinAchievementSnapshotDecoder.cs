...
public sealed class GenshinAchievementSnapshotDecoder
{
    ...
    private readonly GenshinAchievementProtocolProfile _profile;

    public GenshinAchievementSnapshotDecoder(
        AchievementCatalog catalog,
        string gameVersion,
        GenshinAchievementProtocolProfile profile
    )
    {
    ...
    private SnapshotCandidate? EvaluateIdField(CapturedPacket packet, RecordCollection collection, uint idFieldNumber)
    {
        ...
        // 原神成就 ID 目前都在 8xxxx。目录命中率用来排除其它同样是五位数
        // 的重复消息，不能只靠数值区间。
